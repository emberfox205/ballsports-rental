  ...
  .then((mediaStream) => {
    ...
    const takeSnapshot = () => {
      ...
      fetch(endpoint, {
        ...
        .then(data => {
          var formattedData = fixFormat(data);
          updateBallDetection(formattedData);
          updateStatusDetection(formattedData);
          console.log(data);
          if ('redirect' in data) {
    ...
    function fixFormat(response) {
      ...
      // Fix ball names' capitalization and other formatting
      if (formalBallNames.hasOwnProperty(response.ball_name)) {
        formattedResponse.ball_name = formalBallNames[response.ball_name];
      }

      // Round accuracy 
      if (typeof response.confidence === 'number') {
        formattedResponse.confidence = response.confidence.toFixed(4);
      }
    ...
    function detectionRedirect(response) {
      // Store detection results into localStorage
      localStorage.setItem('ballType', response.ball_name);
      localStorage.setItem('accuracy', response.confidence);
